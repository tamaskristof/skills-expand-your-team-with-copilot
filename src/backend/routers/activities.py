...
def get_activities(
    ...
    - difficulty: Filter by difficulty level ('Beginner', 'Intermediate', 'Advanced').
                  Activities with no difficulty set are included in all levels.
    """
    ...
    if difficulty:
        # Include activities with matching difficulty OR with no difficulty (all levels)
        # Wrap with $and to safely combine with any other active filters
        difficulty_filter = {"$or": [
            {"difficulty": difficulty},
            {"difficulty": {"$exists": False}}
        ]}
        if query:
            query = {"$and": [query, difficulty_filter]}
        else:
            query = difficulty_filter
